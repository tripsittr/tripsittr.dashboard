...
use App\Filament\Clusters\Music\Resources\ReleasedResource\Pages;
                ...
                ->action(function (Approval $record) {
                    if ($record->approvable instanceof Album) {
                        ...
                        if ($album->user_id && $album->status === 'approved') {
                            optional($album->user)->notify(new \App\Notifications\AlbumApproved($album));
                        } elseif ($album->user_id && $album->status === 'released') {
                            optional($album->user)->notify(new \App\Notifications\AlbumApproved($album));
                            optional($album->user)->notify(new \App\Notifications\AlbumReleased($album));
                        }
                    }
                }),

...
<head>
    ...
    <title>User {{ \App\Models\User::find($userId)?->name ?? 'Unknown User' }} has performed action: {{ $action }}</title>
    <style>
        body {
...
<body>
    ...
    <div class="content">
        ...
        <p>Hello,</p>
        <p>A new model has been created in the system:</p>
        <ul>
            <li><strong>User Name:</strong> {{ $model->title }}</li>
            <li><strong>Created By:</strong> {{ $user->name }}</li>
            <li><strong>Created At:</strong> {{ now()->toDateTimeString() }}</li>
        </ul>
